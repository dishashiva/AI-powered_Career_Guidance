...
FILE1 = os.path.join(BASE, "CareerAI_Milestone_2_Report.docx")
FILE2 = os.path.join(BASE, "Disha S_Milestone 2.docx")

# 1. Ensure images are cropped to 16:9 widescreen landscape ratio (1263 x 710)
img_names = ["Profile management.png", "Resume parsing.png", "Resume upload and delete.png"]
for fname in img_names:
    ...
    if os.path.exists(fpath):
        ...
        if im.height != target_h:
            cropped = im.crop((0, 0, w, target_h))
            cropped.save(fpath)
            print(f"Cropped {fname} to 16:9 ratio: {cropped.width}x{cropped.height}")


def generate_milestone2_grid_doc(output_path):
    doc = Document()
    ...
    add_p(
        "System Workflow: Data Flow Architecture (Milestone 2 Workflow):",
        bold_prefix="Data Flow Architecture: ",
    )
    ...
    # ══════════════════════════════════════════════════════════════
    # 6. OUTPUT & SCREENSHOTS (2-COLUMN 16:9 GRID LAYOUT)
    # ══════════════════════════════════════════════════════════════
    doc.add_heading("6. Output & Screenshots", level=1)
    add_p(
        "Below are the empirical output screenshots demonstrating the implemented Milestone 2 modules in a 16:9 landscape grid view."
    )

    # Create a 2-Column Grid Table
    ...
    items = [
        (
            ...
            "Figure 1: User Profile Management & AI Autofill Module",
            0,
            0,
        ),
        (
            "Resume parsing.png",
            "Figure 2: AI Resume Parsing & 95%+ Metadata Extraction Module",
            0,
            1,
        ),
        (
            "Resume upload and delete.png",
            "Figure 3: Resume Management (Upload, Status, View, Delete)",
            1,
            0,
        ),
    ]

    for filename, caption, r_idx, c_idx in items:
        cell = grid_table.rows[r_idx].cells[c_idx]
        ...
        if os.path.exists(img_path):
            run = p_cell.add_run()
            # 7.6 cm width fits 2-column A4 grid perfectly with 16:9 ratio
            run.add_picture(img_path, width=Cm(7.6))

        p_cap = cell.add_paragraph(caption, style="Caption")
        p_cap.alignment = WD_ALIGN_PARAGRAPH.CENTER
        p_cap.paragraph_format.space_before = Pt(2)
        p_cap.paragraph_format.space_after = Pt(6)

    # Fill cell (1,1) with a verification badge card
    c_last = grid_table.rows[1].cells[1]
    p_last = c_last.paragraphs[0]
    p_last.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_last.paragraph_format.space_before = Pt(12)
    r_l1 = p_last.add_run("✅ Milestone 2 System Verified\n")
    r_l1.bold = True
    r_l1.font.size = Pt(11)
    r_l1.font.color.rgb = PRIMARY_BLUE

    p_info = c_last.add_paragraph()
    p_info.alignment = WD_ALIGN_PARAGRAPH.CENTER
    r_info = p_info.add_run(
        "All 3 core modules (Profile Management, Resume Parsing, and Resume CRUD) are fully operational and validated."
    )
    r_info.font.size = Pt(9.5)
    r_info.font.color.rgb = GRAY_TEXT
    ...
    try:
        ...
        print("Saved grid document to:", output_path)
    except PermissionError:
        alt_path = output_path.replace(".docx", "_Grid.docx")
        doc.save(alt_path)
        print("Notice: Primary file locked by Word. Saved grid document to:", alt_path)


generate_milestone2_grid_doc(FILE1)
generate_milestone2_grid_doc(FILE2)
